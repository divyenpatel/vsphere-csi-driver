...
import (
	...
	"sigs.k8s.io/vsphere-csi-driver/v2/pkg/csi/service/osutils/windows"
)
...
func (APIImplementor) GetService(name string) (*ServiceInfo, error) {
	...
	out, err := windows.RunPowershellCmd(script, cmdEnv)
	if err != nil {
...
func (APIImplementor) StopService(name string, force bool) error {
	...
	out, err := windows.RunPowershellCmd(script, fmt.Sprintf("ServiceName=%s", name), fmt.Sprintf("Force=%t", force))
	if err != nil {
